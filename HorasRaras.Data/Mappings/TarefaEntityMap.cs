...
public class TarefaEntityMap : IEntityTypeConfiguration<TarefaEntity>
{
    public void Configure(EntityTypeBuilder<TarefaEntity> builder)
    {
        builder.HasKey(x => x.Id);
        builder.HasOne(x => x.Projeto).WithMany(x => x.Tarefas).OnDelete(DeleteBehavior.Restrict);
        builder.Property(x => x.HorasTotal).HasConversion<float>();
        //# AVALIAR ESSA RELAÇÃO
        builder.HasOne(x => x.Usuario).WithMany(x => x.Tarefas).OnDelete(DeleteBehavior.Restrict);
